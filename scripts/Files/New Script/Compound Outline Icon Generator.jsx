/**
 * Compound Outline Icon Generator for Adobe Illustrator
 * 
...
if (app.name !== "Adobe Illustrator") {
 ...
 function calculateInnerStroke(weight, size, baseStroke100, increment, isIncrease) {
   // Calculate stroke for weight 100 at target size
   var baseStrokeScaled = baseStroke100 * (size / 32);
   
   // Calculate how many 100-weight units above/below 100
   var weightUnits = (weight - 100) / 100;
   
   // Apply increment/decrement
   var incrementScaled = increment * (size / 32);
   var adjustment = isIncrease 
     ? weightUnits * incrementScaled 
     : -weightUnits * incrementScaled;
 ...
 function applyStrokeWidthWithInner(item, weight, size, baseStroke100, increment, isIncrease, isTopLevel, outerSize) {
   try {
       // Calculate inner stroke for this weight and size
       var innerStrokeWidth = calculateInnerStroke(weight, size, baseStroke100, increment, isIncrease);
       var outerStrokeWidth = calculateStroke(weight, size);
       
       if (item.typename === 'PathItem') {
           if (item.stroked) {
               // Determine if this is an inner element
               var isInner = false;
               if (!isTopLevel && outerSize > 0) {
                   var bounds = item.geometricBounds;
                   if (bounds && bounds.length === 4) {
                       var width = bounds[2] - bounds[0];
                       var height = bounds[1] - bounds[3];
                       var elementSize = Math.max(width, height);
                       isInner = elementSize < outerSize * 0.95;
                   }
               }
               item.strokeWidth = isInner ? innerStrokeWidth : outerStrokeWidth;
           }
       } else if (item.typename === 'CompoundPathItem') {
           // For compound paths, check if it's inner based on size
           var isInner = false;
           if (!isTopLevel && outerSize > 0) {
               var bounds = item.geometricBounds;
               if (bounds && bounds.length === 4) {
           ...
           var strokeToApply = isInner ? innerStrokeWidth : outerStrokeWidth;
           for (var i = 0; i < item.pathItems.length; i++) {
               if (item.pathItems[i].stroked) {
                   item.pathItems[i].strokeWidth = strokeToApply;
               }
           }
       } else if (item.typename === 'GroupItem') {
           for (var j = 0; j < item.pageItems.length; j++) {
               applyStrokeWidthWithInner(item.pageItems[j], weight, size, baseStroke100, increment, isIncrease, false, outerSize);
           }
       }
   } catch (e) {
       // Item may be locked or unmodifiable, skip it
 ...
 function showDialog() {
   ...
   // ============================================================================
   var win = new Window("dialog", "Compound Outline Icon Generator");
  ...
  // Size Range Input
  var sizeRangeRow = sizesPanel.add("group");
  sizeRangeRow.orientation = "row";
  sizeRangeRow.spacing = 8;
  sizeRangeRow.alignChildren = ["left", "center"];
  sizeRangeRow.add("statictext", undefined, "Size range (e.g., 8-50):");
  sizeRangeInput = sizeRangeRow.add("edittext", undefined, "");
  sizeRangeInput.characters = 12;
  sizeRangeInput.preferredSize = [100, 22];
  ...
  var sizeGrid = sizesPanel.add("group");
   ...
   // Custom Inner Stroke Options
   customInnerStrokeCb = compoundPanel.add("checkbox", undefined, "Use custom inner stroke width");
   customInnerStrokeCb.value = false;
   
   // Container for inner stroke controls (initially hidden)
   var innerStrokeContainer = compoundPanel.add("group");
   innerStrokeContainer.orientation = "column";
   innerStrokeContainer.alignChildren = ["left", "top"];
   innerStrokeContainer.spacing = 6;
   innerStrokeContainer.visible = false;
   
   // Base stroke width input
   var baseStrokeRow = innerStrokeContainer.add("group");
   baseStrokeRow.orientation = "row";
   baseStrokeRow.spacing = 8;
   baseStrokeRow.alignChildren = ["left", "center"];
   baseStrokeRow.add("statictext", undefined, "Base stroke width (weight 100):");
   innerStrokeBaseInput = baseStrokeRow.add("edittext", undefined, "0.3");
   innerStrokeBaseInput.characters = 8;
   innerStrokeBaseInput.preferredSize = [60, 22];
   baseStrokeRow.add("statictext", undefined, "px");
   
   // Increase/Decrease radio buttons
   var radioContainer = innerStrokeContainer.add("group");
   radioContainer.orientation = "row";
   radioContainer.spacing = 8;
   radioContainer.alignChildren = ["left", "center"];
   
   var radioGroup = radioContainer.add("group");
   radioGroup.orientation = "row";
   radioGroup.spacing = 4;
   innerStrokeIncreaseRadio = radioGroup.add("radiobutton", undefined, "Increase by");
   innerStrokeIncreaseRadio.value = true;
   innerStrokeDecreaseRadio = radioGroup.add("radiobutton", undefined, "Decrease by");
   
   // Increment input
   var incrementRow = innerStrokeContainer.add("group");
   incrementRow.orientation = "row";
   incrementRow.spacing = 8;
   incrementRow.alignChildren = ["left", "center"];
   incrementRow.add("statictext", undefined, "Per 100 weight units:");
   innerStrokeIncrementInput = incrementRow.add("edittext", undefined, "0.1");
   innerStrokeIncrementInput.characters = 8;
   innerStrokeIncrementInput.preferredSize = [60, 22];
   incrementRow.add("statictext", undefined, "px");
   
   // Event handler for checkbox visibility
   customInnerStrokeCb.onClick = function() {
  ...
  // Helper function to parse and expand size ranges
  function parseSizeRange(rangeText) {
      var sizes = [];
      if (!rangeText) {
          return sizes;
      }
      ...
      // Trim whitespace
      var trimmed = String(rangeText).replace(/^\s+|\s+$/g, '');
      if (!trimmed) {
          return sizes;
      }
      
      // Try to detect range patterns: "8-50", "8 - 50", "8–50" (en dash), "8—50" (em dash)
      // Match patterns like: number, optional whitespace, dash (various types), optional whitespace, number
      ...
      if (rangeMatch) {
          ...
          if (start > 0 && end > 0 && !isNaN(start) && !isNaN(end)) {
              // Handle reversed ranges (e.g., 50-8 should become 8-50)
              if (start > end) {
                  var temp = start;
                  start = end;
                  end = temp;
              ...
              // Generate all sizes from start to end (inclusive)
              for (var i = start; i <= end; i++) {
                  sizes.push(i);
              }
          }
      } else {
          // Not a range pattern, try to parse as single number
          var singleValue = parseInt(trimmed, 10);
          if (singleValue > 0 && !isNaN(singleValue)) {
              sizes.push(singleValue);
          }
      }
  ...
  function getSelectedSizes() {
      ...
      // Get sizes from range input
      try {
          var rangeText = sizeRangeInput.text;
          if (rangeText) {
              var rangeSizes = parseSizeRange(rangeText);
              for (var r = 0; r < rangeSizes.length; r++) {
                  sizes.push(rangeSizes[r]);
              }
          }
      } catch (e) {
          // Skip invalid range input
      }
      // Deduplicate
      var uniqueSizes = [];
      var seen = {};
 ...
 function generateArtboards(options) {
   ...
   for (var s = 0; s < options.sizes.length; s++) {
       ...
       for (var w = 0; w < options.weights.length; w++) {
           ...
           // Apply outer stroke width to all elements first
           ...
           if (options.customInnerStroke) {
               var outerSize = getLargestElementSize(newGroup);
               applyStrokeWidthWithInner(newGroup, weight, size, options.innerStrokeBase, options.innerStrokeIncrement, options.innerStrokeIncrease, true, outerSize);
           }
 ...
 function exportToSVG(options) {
  ...
  // Process sizes in batches of 25 to avoid performance issues
  var batchSize = 25;
  var totalBatches = Math.ceil(options.sizes.length / batchSize);
  var totalExports = options.sizes.length * options.weights.length;
  
  // Helper function to show progress dialog between batches
  function showProgressDialog(lastSize, lastWeight, batchIdx, totalBatches, count, totalExports, isCompleted) {
      var progressDialog = new Window("dialog", "Export Progress");
      progressDialog.orientation = "column";
      progressDialog.alignChildren = ["fill", "top"];
      progressDialog.spacing = 12;
      progressDialog.margins = 16;
      progressDialog.preferredSize = [400, 200];
      
      // Title
      var titleText = progressDialog.add("statictext", undefined, "Exporting SVG Files...");
      ...
      titleText.preferredSize = [-1, 24];
      
      // Progress bar
      var progressBar = progressDialog.add("progressbar", undefined, 0, 0, 100);
      progressBar.preferredSize = [-1, 20];
      progressBar.alignment = ["fill", "center"];
      
      // Calculate progress percentage
      var progressPercent = totalExports > 0 ? Math.round((count / totalExports) * 100) : 0;
      progressBar.value = progressPercent;
      
      // Last exported info
      var lastExportText = progressDialog.add("statictext", undefined, 
          isCompleted ? "Export completed!" :
          "Last exported: Size " + lastSize + "px, Weight " + lastWeight);
      lastExportText.graphics.font = ScriptUI.newFont(lastExportText.graphics.font.name, ScriptUI.FontStyle.BOLD, 12);
      lastExportText.preferredSize = [-1, 30];
      lastExportText.alignment = ["center", "center"];
      
      // Progress text
      var progressText = progressDialog.add("statictext", undefined, 
          "Batch " + batchIdx + " of " + totalBatches + " completed\n" +
          "Exported " + count + " of " + totalExports + " files (" + progressPercent + "%)");
      progressText.preferredSize = [-1, 50];
      progressText.alignment = ["center", "center"];
      
      // Status message
      var statusText = progressDialog.add("statictext", undefined, 
          isCompleted ? "All files exported successfully!" :
          "Click Continue to process next batch, or Cancel to stop.");
      statusText.preferredSize = [-1, 30];
      statusText.alignment = ["center", "center"];
      
      // Button group
      var buttonGroup = progressDialog.add("group");
      buttonGroup.orientation = "row";
      buttonGroup.spacing = 8;
      buttonGroup.alignment = ["center", "center"];
      
      var continueBtn = buttonGroup.add("button", undefined, isCompleted ? "Close" : "Continue");
      continueBtn.preferredSize = [100, 28];
      var cancelBtn = buttonGroup.add("button", undefined, "Cancel");
      cancelBtn.preferredSize = [100, 28];
      cancelBtn.enabled = !isCompleted;
  ...
  // Process each batch of sizes automatically
  for (var batchIdx = 0; batchIdx < totalBatches; batchIdx++) {
      var batchStart = batchIdx * batchSize;
      var batchEnd = Math.min(batchStart + batchSize, options.sizes.length);
      var sizeBatch = options.sizes.slice(batchStart, batchEnd);
      
      var lastExportedSize = 0;
      var lastExportedWeight = 0;
      
      // Create a temporary document for each export in this batch (processes automatically)
      for (var s = 0; s < sizeBatch.length; s++) {
          var size = sizeBatch[s];
          
          for (var w = 0; w < options.weights.length; w++) {
              var weight = options.weights[w];
              
              // Track last exported size/weight for progress display
              lastExportedSize = size;
              lastExportedWeight = weight;
              var scaleFactor;
              var strokeWidth;
              ...
              if (options.customInnerStroke) {
                  var outerSize = getLargestElementSize(tempGroup);
                  applyStrokeWidthWithInner(tempGroup, weight, size, options.innerStrokeBase, options.innerStrokeIncrement, options.innerStrokeIncrease, true, outerSize);
              }
      ...
      // Show progress dialog after batch completes
      // For all batches (including first), show progress and allow cancellation
      var result = showProgressDialog(lastExportedSize, lastExportedWeight, batchIdx + 1, totalBatches, count, totalExports, false);
      
      // If user cancelled, stop exporting
      if (result === 0) {
          errors.push("Export cancelled by user after batch " + (batchIdx + 1));
          break;
      }
      
      // Give system time to clean up memory before next batch
  ...
  if (errors.length === 0 || count > 0) {
      showProgressDialog(0, 0, totalBatches, totalBatches, count, totalExports, true);
  }
